    ...
    public partial class CreateRequest : UserControl
    {
        ...
        private async void btnGenerate_Click(object sender, RoutedEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Functions.fn_delete_file(tbPathName.Text + "\\" + tbPrivateKeyName.Text + ".key");
            ...
            else
            {
                ...
                {
                    Functions.fn_delete_file(tbPathName.Text + "\\" + tbRequestName.Text + ".csr");
            ...
            else
            {
                ...
                GenerateCertificateRequest(tbDomainName.Text, subjectAlternativeNames, keyLength, cbSinatuteAlgorithm.Text,
                    ...
                    tbPathName.Text, tbPrivateKeyName.Text + ".key", tbRequestName.Text + ".csr", tbEmail.Text, isThisCACert, pbPasswordPrivateKey.Password);
            }
        }
        ...
        private void GenerateCertificateRequest(string commonName, string[] subjectAlternativeNames, int keyLength,
            ...
            string path,string privateFileKeyName, string requestFileName, string email, bool isThisCACert=false, string password=null)
        {
                                                    ...
                                                    )
        {
            ...
            try
            {
                ...
                //  ukladani privKey do databaze
                conStrMysql.ConnectionString = MainWindow.dbConnectioString;
                List<MySqlParameter> arrParameters = new List<MySqlParameter>();


                arrParameters.Add(new MySqlParameter("@privKeyData", privateKeyPem));
                conStrMysql.Open();
                IDataReader Reader = MySqlHelper.ExecuteReader(conStrMysql, String.Format("START TRANSACTION;UPDATE ciselnik_ridici cr SET cr.CERTIFIKAT_KLIC = @privKeyData,CERTIFIKAT_TM = NULL,CERTIFIKAT = NULL,CERTIFIKAT_PLATNOST = NULL  WHERE cr.ID = " + Int64.Parse(commonName.Split('-')[1]).ToString() + ";COMMIT;"), arrParameters.ToArray());
                //Reader.Read();
                conStrMysql.Close();
                // konec ukladani 
                ...
                {
                    Text = "Privátní klíč: " + outputPrivateKeyName + " byl úspěšně uložen." + "\n",
            ...
            try
            {
                ...
                if (isOK)
                {
                    tbOutputMessageBox.Inlines.Add(new Run
                    {
                        Text = "CSR požadavek: " + outputCertReqFile + " byl úspěšně vygenerován." + "\n",
                        Foreground = System.Windows.Media.Brushes.Green
                    });
            ...
            try
            {
                ...
                //  ukladani CSR do databaze
                conStrMysql.ConnectionString = MainWindow.dbConnectioString;
                List<MySqlParameter> arrParameters = new List<MySqlParameter>();
                try
                {
                    byte[] csrData;
                    FileStream fs = new FileStream(outputCertReqFile, FileMode.Open, FileAccess.Read);
                    BinaryReader br = new BinaryReader(fs);
                    csrData = br.ReadBytes((int)fs.Length);
                    br.Close(); fs.Close();

                    arrParameters.Add(new MySqlParameter("@csrData", csrData));

                    conStrMysql.Open();
                    IDataReader Reader = MySqlHelper.ExecuteReader(conStrMysql, String.Format("START TRANSACTION;UPDATE ciselnik_ridici cr SET cr.CERTIFIKAT_CRS = @csrData,CERTIFIKAT_TM = NULL,CERTIFIKAT = NULL,CERTIFIKAT_PLATNOST = NULL   WHERE cr.ID = " + Int64.Parse(commonName.Split('-')[1]).ToString() + ";COMMIT;"), arrParameters.ToArray());
                    //Reader.Read();
                    conStrMysql.Close();
                ...
                tbOutputMessageBox.Inlines.Add(new Run
                {
                    Text = "Soubor CSR : " + outputCertReqFile + " byl úspěšně uložen." + "\n",
                    Foreground = System.Windows.Media.Brushes.Green
            ...
            //Functions.SendMailWithMailTo("epodatelna@t-mobile.cz",
            // "Žádost o vygenerování certifikátu",
            // "Dobrý den, Žádám o vygenerování certifikátu z přiloženého CSR souboru.",
            // outputCertReqFile.Replace("\\","//"));
        ...
        private void InstallCertificateLocal(string cerFileName, string cerPass)
        {
            X509Certificate2 certificate;

            if (String.IsNullOrEmpty(cerPass))
                certificate = new X509Certificate2(cerFileName);
            else
                certificate = new X509Certificate2(cerFileName, cerPass);

            X509Store store = new X509Store(StoreName.My, StoreLocation.LocalMachine); // personal
            X509Store store1 = new X509Store(StoreName.CertificateAuthority, StoreLocation.LocalMachine); // intermediate
            X509Store store2 = new X509Store(StoreName.Root, StoreLocation.LocalMachine); // trusted root certificate authorities
        ...
        private void btnInstCert_Click(object sender, RoutedEventArgs e)
        {
            CertStoreReader csr = new CertStoreReader("D140252.DMS.LOCAL");
            csr.InstallCert(CertStoreName.MY, "webserver_signed1.cer");

            //csr.InstallCert(CertStoreName.MY, "webserver1.pfx");
        }
